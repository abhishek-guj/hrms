...
public interface PostRepository extends JpaRepository<Post, Long> {
    ...
    /** Filter by tag name */
    @Query("SELECT DISTINCT p FROM Post p JOIN p.postTags pt JOIN pt.tag t WHERE p.isDeleted = false AND t.name = :tagName ORDER BY p.createdDate DESC, p.id DESC")
    ...
    /** Check if a system birthday/anniversary post already exists for today to avoid duplicates */
