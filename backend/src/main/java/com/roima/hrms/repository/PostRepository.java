...
public interface PostRepository extends JpaRepository<Post, Long> {
        ...
        /** Filter by author */
        @Query("SELECT p FROM Post p WHERE p.isDeleted = false AND p.author.id = :authorId ORDER BY p.createdDate DESC, p.id DESC")
        ...
        @Query("SELECT p FROM Post p WHERE p.isDeleted = false AND p.author.id = :authorId AND p.createdDate BETWEEN :from AND :to ORDER BY p.createdDate DESC, p.id DESC")
         ...
         * duplicates
         */
        @Query("SELECT COUNT(p) > 0 FROM Post p WHERE p.isSystemGenerated = true AND p.author.id = :employeeId AND p.createdDate = :today AND p.title LIKE :titleLike")
